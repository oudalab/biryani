    ...
    public static void main(String[] argv) throws Exception
    {
            ...
            public void run() {
                ...
                if(!instance.batch_timer.isRunning() && last_index>0 && instance.flush_timer.elapsed(TimeUnit.MILLISECONDS)>=60000)
                {
                    try
                    {
                        ...
                        last_index=0;
                        envArrayList.clear();
			instance.log.debug(log_token+" Restarting the container as we reached the flush timer");
			System.exit(4);
                    }
                    catch (IOException e)
                    {
        ...
        try
        {
            ...
            instance.consumer_rabbimq = new DefaultConsumer(instance.channel)
            {
                                           ...
                                           AMQP.BasicProperties properties, byte[] body) throws IOException
                {
                    ...
                    finally
                    {
                        ...
                        {
                            instance.log.debug("Exception due to Rabbitmq");
                            System.exit(3);
                        }
                    }
                }
            ...
            if(instance.initial_start)
            {
                ...
                System.out.println(instance.consumer_tag+":Started");
                instance.log.debug(log_token+" Consumer_tag: "+instance.consumer_tag+": Started");
                //instance.initial_start=false;
            }
            ...
            if(--instance.main_avg_count==0)
            {
                ...
                if(instance.batch_timer.elapsed(TimeUnit.SECONDS)- instance.expected_batch_time > 2*instance.exptected_time_raiser)
                {
                    ...
                    if(temp_batch!=0)
                    {
                        ...
                        instance.optimal_batch_test=0;
                        instance.log.debug(log_token+" Optimal Batch size: "+instance.optimal_batch);
                        System.out.println(log_token+" Optimal Batch size: "+instance.optimal_batch);
                    }
